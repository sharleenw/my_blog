...
About two years ago, I was taking my dog for a walk through a park and I began to notice the birds and how fascinating they are! :bird: I began regularly going out birding (or "bird-watching") and reading up on these cool little flying dinosaurs.
...
More specifically, I've been curious about bird counts in general, because birders will say things like "the House Sparrows are getting worse every year" or "the number of Bald Eagles has increased", and I was wondering if the Christmas Bird Count data could confirm or deny some of these statements.
...
I read in the data using the [`readr`](https://readr.tidyverse.org/) package.
...
As shown below, it turns out that the first two rows of data just give information about the count name and latitude/longitude, so I extracted those two pieces of information as `current_circle_name` and `lat_long` and then [`slice`d](https://dplyr.tidyverse.org/reference/slice.html) the file so that the first two lines were excluded from the dataset. I then used `clean_names` from the [`janitor`](https://www.rdocumentation.org/packages/janitor/versions/1.1.1/topics/clean_names) package.
...
Since I played around with the data before writing this, I knew that there were actually five tables in this dataset.

The first three tables contain count day weather data. A lot of the weather data is missing and I'm not sure what the units are for the temperature column (sometimes it's hard-coded in as Celsius or Fahrenheit but sometimes there's no units). I will remove these three tables from the bird count data table.

In order to get an idea of what I mean, here are snapshots between each table:

Here is the end of the first table and the start of the second table. Notice that there is a line of `NA`s between the two tables:
...
Here is the end of the second table and the start of the third table. Notice that there is a line of `NA`s between the two tables:
...
Then, using the `replace_with_na_all` function from the [`naniar`](https://purrr.tidyverse.org/) package, anywhere the data was recorded as "Unknown", I converted it to a proper `NA`.
...
We are almost able to clean the bird count data!

But first, we need to deal with the fact that there is a fifth table below the bird count data table (the fourth table). Here is the end of the fourth table and the start of the fifth table. Notice that there is a line of `NA`s between the two tables:
...
I won't show the data of the fifth table here, but it shows the full names of the counters. I won't add this data to the `overall_meta_table`, so I will just remove the table from the dataset.

Before removing the fifth table, we'll clean up the bird count dataset a bit.
...
So where are we at, at the moment? We have only one dataset left, and it looks like this:
...
- `number_by_party_hours` is how many birds were counted divided by the number of person-hours that year (as per the Christmas Bird Count [documentation](https://www.audubon.org/sites/default/files/documents/compilers_manual_0.pdf))
- `flags` contains values like "`US`" for "unusual" bird
...
And let's take another look at where we're at with the `hamilton_cbc` dataset:
...
And that's it! :smile: :tada: We have cleaned the dataset and are ready to do some visualising :eyes: in Part 2!
