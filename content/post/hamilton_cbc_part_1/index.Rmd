...
Since I played around with this dataset before writing this, it turns out that there are actually five tables in this dataset: The first three tables contain weather data of the count day. I determined that a lot of the weather data is missing and I'm not sure what the units are for the temperature column (sometimes it's hard-coded in as Celsius or Fahrenheit and sometimes there's no units). Since I will be doing another blog post about historical weather trends, I will remove these first three tables from the dataset.
...
hamilton_cbc %>% slice(23215:23221)
...
So where are we at, at the moment? We have only one data set left, and it looks like this:
...
- `count_year` data gives a lot of information that we will parse out in a moment
- `how_many_cw` provides the actual bird count
- `number_by_party_hours` is (as per the CBC [documentation](https://www.audubon.org/sites/default/files/documents/compilers_manual_0.pdf)) how many birds were counted divided by the number of person-hours that year
...
I use the regex I found on [StackOverflow](https://stackoverflow.com/questions/8613237/extract-info-inside-all-parenthesis-in-r), which as Nettle says:
...
Here we are putting the two `mutate`s together:
...
Now we will look at the `count_year` variable. Let's get a sense of what the variable looks like, using the White-Breasted Nuthatch count of 2016:
...
This is all metadata and we can take it out of this data set and put it in our `overall_meta_data` file. The only variable we will keep in the `hamilton_cbc` data set is the calendar year, so that we can plot by year and so we can join to the metadata later if we want.
...
count_participant_meta_data <- hamilton_cbc %>%
  ...
  mutate(number_of_participants = str_extract(
    participant_info, "(?<=Participants:\\s).+?(?=\\s#)")) %>%  # Getting everything between "Participants: " and " #"
  mutate(species_reported = str_extract(
    participant_info, "(?<=Reported:\\s).+?(?=\\nTotal)")) %>%  # Getting everything between "Reported: " and "Total"
  mutate(total_hours = str_extract(
    participant_info, "(?<=Hrs\\.:\\s).*$"))  # Getting everything after "Hrs.: "
# This regex is different because we are going until the end of the string:
# https://forum.sublimetext.com/t/regex-match-everything-after-this-word/20764

count_participant_meta_data %>% head()

```

Since `number_of_participants` and `species_reported` ended up having lots of missing data, I decided not to include them in the `overall_meta_data` data set.
...
overall_meta_data %>% glimpse()

```

Let's take another quick peek at where we're at with the `hamilton_cbc` data set:

```{r where_are_we}

hamilton_cbc %>% glimpse()

```

Let's just clean up the variables:
...
We almost have a clean data set! :soon: :sparkles::sparkles::sparkles: :tada:

I am going to remove the `flags` variable (`flags` refers to things like "`US`" for "unusual" bird, which we don't need for our analysis). I am also going to remove `number_by_party_hours` and derive it myself instead.
...
It turns out that `how_many_counted` also has a "`cw`" value, which means the bird was not seen on count day itself, but was seen during the days leading up to the count, or right after. I am going to set these bird counts as `NA`, as they don't have a specified value that I can be sure of (it's probably a "1", but I can't be sure).
...
In the `species` variable, there are some rows that are identified only to the species level. I think I will exclude them. I believe [EBird](https://ebird.org/home) excludes them too.
...
- Using `tidyr`'s [`replace_na`](https://rdrr.io/cran/tidyr/man/replace_na.html) function, let's make all of the `NA`s equal to 0 for `how_many_counted`. So we are assuming that all birds in the area were seen by counters on count day.
- Let's calculate the number of birds (within a species) seen divided by the total number of count hours that happened that year.
...
And that's it! :smile: :tada: We have cleaned the data set and are ready to do some visualising :eyes: in Part 2!

Here is a glimpse of our final data set:
...
hamilton_cbc %>% write_rds("hamilton_cbc_output.rds")
