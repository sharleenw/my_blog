...
The second step is to sum up the counts for each year across all of the subspecies so the counts are the same, and then filter to only keep the first instance of each `species` (which, when arranged alphabetically, is the shortest species name):
...
Perfect! No more subspecies. The last group of species to deal with is `species` where the name has either a `(` or a `/`:
