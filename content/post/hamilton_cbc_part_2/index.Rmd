...
2. consolidate the names of some species that have changed back and forth since 1921.
...
So now, onto cleaning the trickier stuff. Sometimes, species have sub-species names or groups, for example:
...
I just want there to be one Dark-eyed Junco species in this dataset, so I am going to consolidate these four "species" into one species. Even though I know people get way more excited about seeing the Oregon sub-species in Hamilton than the Slate-colored :smile:.
...
The second step is to sum up the counts for each year across all of these subspecies, and then only keep the first `species` in each `species_latin` group (which, alphabetically, is the shortest species name):
...
The last group of species to deal with is where either `species` has a `(` or a `/` or `species_latin` has a `/`:
...
# Recalculate the "per person-hour" variable that depends on how_many_counted
hamilton_cbc <- hamilton_cbc %>%
...
102 species, nice! It's not easy to get to 100 species during a CBC. Only four other Ontario CBCs have done it, at least by 2004 (the latest year of CBC data in @hsa_birds).
...
According to @hsa_birds, Rock Pigeons only started being counted in 1973 and House Sparrows in 1931 (possibly due to them being introduced).
