...
Now, onto cleaning the trickier stuff. Sometimes, species have sub-species names or groups, for example, that have different total counts:
...
The second step is to sum up the counts for each year across all of the subspecies so the counts are the same, and then filter to only keep the first instance of `species` (which, when arranged alphabetically, is the shortest species name):
...
Perhaps the nest boxes are helping! Bluebird numbers have certainly increased since the 1980's. And what interesting regular increases and decreases since the late 1990's!

However, in @hsa_birds, there is a chapter by Bill Read called *History of the Eastern Bluebird in the Hamilton Study Area* where he believes that the reason more bluebirds are overwintering now is mainly due to warming winter weather patterns.
...
hamilton_cbc %>%
  plot_species(start_year = 1955,
               species_to_plot = c("Carolina Wren", "Marsh Wren", "Winter Wren"))

```

In @hsa_birds, he states that climate warming with less snow cover has allowed the Carolina Wren to over-winter here in the more recent years. Possibly the Marsh Wren numbers have decreased due to less marshes in the Hamilton area.
...
I picked a sampling of ducks that show a trend of increaseing since about the mid-1990's. Ian Richards believes that these increases are due to warmer winters (with an increase in open water), and for ducks like the Greater and Lesser Scaup, the introduced Zebra Mussel in Lake Ontario.

Some of these increases are absolutely increadible! The Long-tailed Duck went from practically non-existent up to 25,000 in the mid-2000's!

This is also a good reminder for me that the winter birds I think are "normal" to Hamilton (like Mallards and Long-tailed Ducks) may actually be a quite recent phenomenon.

### Canada Goose

In fact, another bird that is very common now is the Canada Goose, which, as I was looking at the data, learned that it was not always so common!
...
@hsa_birds talks about the Canada Goose's history. It originally was considered a "symbol of wild, untamed Canada" and in the late 1800's, Canada Geese avoided the Hamilton area due to "their haunts [being] invaded by trolley cars, electric lights, telegraph wires and other innovations" (a quote by Thomas McIlwraith in 1886). In the 1960's, people decided to introduce the largest, heaviest sub-species of the Canada Goose into southern Ontario, because it would make Ontario "richer". However, with few natural predators, the Giant Canada Goose has increased in numbers and now degrades habitat for other species and competes with other, migratory sub-species.:expressionless:
...
hamilton_cbc %>%
  filter(str_detect(species_latin, "Larus"),
         !str_detect(species, "Thayer")) %>% # Removing Thayer's Gull due low counts
  plot_species(start_year = 1955,
               species_to_plot = .[["species"]]) # Grabs the species column from the dataset

```

I plotted the gulls partly because I thought it would be neat to filter for species using a genus (in this case, "Larus"). I think the random spike in in Iceland Gull numbers is interesting, and also that the Herring Gull numbers have decreased (@hsa_birds mentions they were actually at an all-time high in the 1960's due to unsanitary landfill practices at the time and have now stabilized). Ring-billed Gull numbers have also increased.

### Effects of DDT

Two species I had often heard having been effecting by the pesticide DDT were the Double-crested Cormorant and the Bald Eagle. DDT was first used in 1947 and was finally banned in 1973. Apparently by 1980, Bald Eagles were experiencing complete reproductive failure in southern Ontario (due to the eggshell thinning).

Do the numbers match up with DDT use?

(Also, if anyone knows a better way to put a label on both facets of a graph, please let me know!
...
Cool! The numbers do increase after DDT was banned in Canada.

### Corvids

``` {r corvids}
# American crow declining on pg 306
# Are ravens returning to their historical area?
hamilton_cbc %>%
  plot_species(start_year = 1955,
...
I decided to plot some corvids to see how their numbers have been doing, especially since the West Nile virus happened in the early 2000's. I remember having thousands of crows in my neighbourhood when I was younger, and our neighbourhood even paid a guy to come with a trained hawk to try to scare them away. Nothing got them to leave until West Nile happened.
...
I personally have never seen a Snow Bunting, but I just think their count numbers are so cool! They are an Arctic bird, so what in the world was happening in the Arctic in the 1970's to cause such huge spikes!? Ahh, such a great mystery!

Also, in the 1800's, they used to be called Snowflakes and there is this adorable quote by Thomas McIlwraith in 1886 in @hsa_birds, saying "all our Canadian boys and girls are familiar with the Snowflake, which is frequently the only one they see when out for the first sleigh ride of the season." :smile:
...
I plotted the Northern Cardinal numbers since 1921. Cardinals used to only be Canada occasionally in Point Pelee (Canada's most southern point), where the first recorded breeding happened in 1901. According to @pp_birds, the Point Pelee locals used to call these unusually-coloured birds "war birds"! :bird: Now they are quite common in the Hamilton area through the winter, with @hsa_birds giving the reasons of climate warming (causing less snow depth), increasing edge habitats where they like to live (due to deforestation) and more bird-feeder stations. :chestnut::sunflower:


In the next blog post, I will be making a :sparkles:Shiny app:sparkles: so that anyone can dig into the data!
